...
abstract: >
  ...
  ChIP-seq data. The package MAnorm2 is developed for quantitatively comparing
  ...
  hierarchical strategy for normalization of ChIP-seq data, and it assesses
  ...
  within-group variability between groups. This vignette explains the working
  principle of MAnorm2 1.0.0 and demonstrates the use of it.
output:
...
vignette: >
  %\VignetteIndexEntry{MAnorm2 for quantitatively comparing groups of ChIP-seq samples}
...
The capability of MAnorm2 results primarily from two basic utilities
implemented in it. One is for the normalization of ChIP-seq data, and the other
 ...
 * differential ChIP-seq analysis between groups of samples;
...
intervals occupied by both of the two samples. For normalization of a set of
...
factor is closest to 0 as baseline. In practice, you may want to avoid using
a sample with bad quality as baseline by explicitly specifying the `baseline`
argument of `normalize()`. Check some information regarding the performed
normalization by
...
Here we give the standard workflow for calling hypervariable ChIP-seq signals
across a set of samples. We use all the H3K27Ac ChIP-seq samples as example:
...
hypervariable ChIP-seq signals across samples, and the others suggest very
stable (i.e., invariant) ChIP-seq signals:

```{r plotVarTestBioCond}
# Visualize the overall test results.
plot(res, pval = 0.01, col = c("#0000000A", "#FF000040"))
```

Note also that the *p*-values derived by `varTestBioCond()` are relatively
conserved, as we currently use all the genomic intervals that are used to fit
MVC to estimate the number of prior degrees of freedom, which means we haven't
considered the influence of hypervariable and invariant intervals (see
[@robustLimma] for a detailed discussion of this topic).
...
Compared with the genomic intervals occupied by all ChIP-seq samples, those
intervals occupied only by some of the samples should be associated with higher
variability across the samples and, thus, tend to have more significant
*p*-values:

```{r partiallyOccupiedIntervals}
n <- rowSums(norm[9:13])
...
a set of ChIP-seq samples. We use all the H3K27Ac ChIP-seq samples as example:
...
could be interpreted as average (absolute) difference in signal intensity
across genomic intervals. In the above example, the average fold change of
H3K27Ac ChIP-seq signal between the 1st and 2nd replicates of GM12891 is about
$2^{0.3024156} \approx 1.2$, while the average fold change between the 1st
replicates of GM12891 and GM12892 is about $2^{0.9229330} \approx 1.9$.
...
Mushan Li, Fengxiang Tan, Haojie Chen.
